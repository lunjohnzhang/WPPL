    ...
    for (int j=0;j<iterations;++j){
        ...
        for (int bid=0;bid<num_batches;++bid){
            ...
            for (int i=start_idx;i<end_idx;++i) {
                ...
                #pragma omp critical 
                {
                    // std::cerr<<"agent "<<agent_idx<<" found a path with goal state: "<<goal_state->pos<<" "<<goal_state->t<<std::endl;
            ...
            for (int i=0;i<goal_states.size();++i) {
                ...
                // std::cerr<<start_idx<<" "<<end_idx<<" "<<p.first<<" "<<p.second->pos<<" "<<p.second->t<<std::endl;
            }

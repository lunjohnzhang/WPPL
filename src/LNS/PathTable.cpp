...
void PathTable::deletePath(int agent_id, const Path& _path,bool verbose)
{
    ...
    // TODO: when we use window size, we ignore this for now, maybe we can use a better data structure such as ordered_set/heap to maintain the makespan. 
...
void PathTable::insertPath(int agent_id, const Parallel::Path& _path, bool verbose)
{
    ...
    // TODO: check whether we need maintain goals and makespan in the life-long setting
    // assert(goals[path.back().location] == MAX_TIMESTEP);
...
bool PathTable::constrained(int from, int to, int to_time, std::vector<int> & ignored_agents) const
{
    ...
    // TODO: we just ignore this for now in our lifelong setting with a planning window.
    // if (!goals.empty())
    // {
    //     if (goals[to] <= to_time)
    //         return true; // target conflict

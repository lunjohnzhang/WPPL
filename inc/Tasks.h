...
struct Task
{
    ...
    Task(int task_id, int location, int t_assigned, int agent_assigned, bool recirc = false, int package_dest = -1, int assigned_chute = -1):
        task_id(task_id), location(location),
        t_assigned(t_assigned), agent_assigned(agent_assigned), recirc(recirc),

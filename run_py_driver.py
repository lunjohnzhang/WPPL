...
def main(map_filepath, seed=0):
    ...
    ret = py_driver.run(
        ...
        end_pt_dist="dist_workstation_721", # one of ["uniform", "dist_workstation_721"]
    )
    ...
    print("num_task_finished", analysis["num_task_finished"])
    ...
    # endpoint_weights = np.array(analysis["endpoint_weights"]).reshape(33, 36)
    # fig, ax = plt.subplots()
    endpoints = np.zeros(33 * 36)
    for e, w in zip(analysis["endpoints"], analysis["endpoint_weights"]):
        endpoints[e] = w

    fig, ax = plt.subplots()
    print(endpoints.reshape(33, 36))
    sns.heatmap(endpoints.reshape(33, 36), ax=ax, vmax=0.2, cmap="Reds")
    fig.savefig("endpoints.png")
